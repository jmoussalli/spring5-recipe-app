...
/**
 * Created by jerom on 13/01/2021 at 16:30
 */
@Slf4j
@Component
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes() {
        ...
        if (!eachUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted Each UOM Not Found");
        }

        Optional<UnitOfMeasure> tableSpoonUomOptional = unitOfMeasureRepository.findByDescription("TableSpoon");

        if (!tableSpoonUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted TableSpoon UOM Not Found");
        }

        Optional<UnitOfMeasure> teaSpoonUomOptional = unitOfMeasureRepository.findByDescription("TeaSpoon");

        if (!teaSpoonUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted TeaSpoon UOM Not Found");
        }

        Optional<UnitOfMeasure> dashUomOptional = unitOfMeasureRepository.findByDescription("Dash");

        if (!dashUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted Dash UOM Not Found");
        }

        Optional<UnitOfMeasure> pintUomOptional = unitOfMeasureRepository.findByDescription("Pint");

        if (!pintUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted Pint UOM Not Found");
        }

        Optional<UnitOfMeasure> cupUomOptional = unitOfMeasureRepository.findByDescription("Cup");

        if (!cupUomOptional.isPresent()) {
            throw new RuntimeException(" Excepted Cup UOM Not Found");
        }
        ...
        if (!americanCategoryOptional.isPresent()) {
            throw new RuntimeException(" Excepted American Category Not Found");
        }

        Optional<Category> mexicanCategoryOptional = categoryRepository.findByDescription("Mexican");

        if (!mexicanCategoryOptional.isPresent()) {
            throw new RuntimeException(" Excepted Mexican Category Not Found");
        }
        ...
        guacRecipe.getIngredients().add(new Ingredient("Mince Red Onion or thinly sliced Green Onion", new BigDecimal(2), tableSpoonUom));
